...
class DecodeIO2BypassPkt extends Module {
  val io = IO(new Bundle {
    val in = Input(Vec(2, Flipped(Decoupled(new DecodeIO))))
    val BypassPktTable = Input(Vec(10, Flipped(Decoupled(new BypassPkt))))
    val issueStall = Output(Vec(2, Bool()))
    val out0 = Output(Decoupled(new BypassPkt))
    val out1 = Output(Decoupled(new BypassPkt))
  })
  ...
  for (i <- 9 to 0) {
    when(io.in(0).bits.ctrl.rfSrc1 === io.BypassPktTable(i).bits.decodePkt.rd && io.BypassPktTable(i).bits.decodePkt.rdvalid && io.BypassPktTable(i).valid){
      i0rs1hitStage := i.asUInt()}
    when(io.in(0).bits.ctrl.rfSrc2 === io.BypassPktTable(i).bits.decodePkt.rd && io.BypassPktTable(i).bits.decodePkt.rdvalid && io.BypassPktTable(i).valid){
  ...
  io.out0.bits.decodePkt.subalu := (i0decodePkt.alu && i0rs1hitStage >= 0.U && i0rs1hitStage <= 3.U && (io.BypassPktTable(i0rs1hitStage).bits.decodePkt.mul || io.BypassPktTable(i0rs1hitStage).bits.decodePkt.load)
    || i0decodePkt.alu && i0rs2hitStage >= 0.U && i0rs2hitStage <= 3.U && (io.BypassPktTable(i0rs2hitStage).bits.decodePkt.mul || io.BypassPktTable(i0rs2hitStage).bits.decodePkt.load)
    || i0decodePkt.alu && i0rs1hitStage >= 0.U && i0rs1hitStage <= 5.U && io.BypassPktTable(i0rs1hitStage).bits.decodePkt.subalu
    || i0decodePkt.alu && i0rs2hitStage >= 0.U && i0rs2hitStage <= 5.U && io.BypassPktTable(i0rs2hitStage).bits.decodePkt.subalu
    )

  io.out1.bits.decodePkt.subalu := (i1decodePkt.alu && i1rs1hitStage >= 1.U && i1rs1hitStage <= 3.U && (io.BypassPktTable(i1rs1hitStage).bits.decodePkt.mul || io.BypassPktTable(i1rs1hitStage).bits.decodePkt.load)
    || i1decodePkt.alu && i1rs2hitStage >= 1.U && i1rs2hitStage <= 3.U && (io.BypassPktTable(i1rs2hitStage).bits.decodePkt.mul || io.BypassPktTable(i1rs2hitStage).bits.decodePkt.load)
    || i1decodePkt.alu && i1rs1hitStage >= 1.U && i1rs1hitStage <= 5.U && io.BypassPktTable(i1rs1hitStage).bits.decodePkt.subalu
    || i1decodePkt.alu && i1rs2hitStage >= 1.U && i1rs2hitStage <= 5.U && io.BypassPktTable(i1rs2hitStage).bits.decodePkt.subalu
    || i1hiti0 && (i0decodePkt.mul || i0decodePkt.load) && i1decodePkt.alu
    )

  //issue stall
  //这部分暂时先只考虑整数指令需要stall的情况（额，整数指令好像这一步不会stall)
  io.issueStall := Seq(false.B, false.B)
  //BypassCtl
  val FuType = VecInit(Seq.fill(10)(0.U.asTypeOf(new decodePkt)))
  for(i <- 0 to 0) FuType(i) := io.BypassPktTable(i).bits.decodePkt
  val Valid = VecInit(Seq.fill(10)(false.B))
  for(i <- 0 to 0) Valid(i) := io.BypassPktTable(i).valid
  //BypassPkt out0
...
class PktPipeline extends Module{
  val io = IO(new Bundle{
    val i0In = Input(Flipped(Decoupled(new BypassPkt)))
    val i1In = Input(Flipped(Decoupled(new BypassPkt)))
    val stallFlush = Input(new StallFlushIO)
    val out = Output(Vec(10,Decoupled(new BypassPkt)))
  })
    //stall point
...
class Bypass extends Module{
  ...
  val PipeLinePkt = Module(new PktPipeline)
  val PipelineCtl = Module(new PipeCtl)
  val DecodeIO2BypassPkt = Module(new DecodeIO2BypassPkt)

  PipeLinePkt.io.i0In <> DecodeIO2BypassPkt.io.out0
  PipeLinePkt.io.i1In <> DecodeIO2BypassPkt.io.out1
  PipeLinePkt.io.stallFlush <> PipelineCtl.pipeCtl

  PipelineCtl.io.i0pipeStall <> DecodeIO2BypassPkt.io.issueStall(1)
  PipelineCtl.io.i1pipeStall <> DecodeIO2BypassPkt.io.issueStall(0)
  PipelineCtl.io.flush <> io.flush
  PipelineCtl.io.memStall <> io.memStall
  DecodeIO2BypassPkt.io.BypassPktTable <> PipeLinePkt.io.out
  DecodeIO2BypassPkt.io.in <> io.in

  io.BypassPkt <> PipeLinePkt.io.out
