...
class SSDIBF extends NutCoreModule with HasInstrType with HasIBUFConst{
  ...
  //RVCExpander
  val expander0 = Module(new RVCExpander)
  val expander1 = Module(new RVCExpander)
  //ibuf decode
  ...
  io.out(0).bits.instr := Mux(io.out(0).bits.isRVC,expander0.io.out.bits,Cat(ringInstBuffer(ringBufferTail+1.U), ringInstBuffer(ringBufferTail)))
  ...
  // io.out(1).bits.pnpc := npcRingMeta(inst2_StartIndex)
  io.out(1).bits.instr := Mux(io.out(1).bits.isRVC,expander1.io.out.bits,Cat(ringInstBuffer(inst2_StartIndex+1.U), ringInstBuffer(inst2_StartIndex)))
  ...
  //RVCExpander
  expander0.io.in := Cat(ringInstBuffer(ringBufferTail+1.U), ringInstBuffer(ringBufferTail))
  expander1.io.in := Cat(ringInstBuffer(inst2_StartIndex+1.U), ringInstBuffer(inst2_StartIndex))
  //dequeue control
...
class RVCExpander extends NutCoreModule{
  val io = IO(new Bundle {
    val in = Input(UInt(32.W))
    val out = Output(new ExpandedInstruction)
  })

  if (SSDCoreConfig().usingCompressed) {
