...
sealed trait HasCacheConst {
    ...
    def getDataIdx(addr: UInt) = Cat(
      ...
      addr.asTypeOf(addrBundle).wordIndex
    )
...
sealed class MetaBundle(implicit val cacheConfig: SSDCacheConfig)
    extends CacheBundle {
...
sealed class DataBundle(implicit val cacheConfig: SSDCacheConfig)
    extends CacheBundle {
...
class SSDCacheIO(implicit val cacheConfig: SSDCacheConfig)
    extends Bundle
    with HasNutCoreParameter
    with HasCacheConst {
...
// meta read
sealed class SSDCacheStage1(implicit val cacheConfig: SSDCacheConfig)
    extends CacheModule {
...
// check
sealed class SSDCacheStage2(implicit val cacheConfig: SSDCacheConfig)
    extends CacheModule {
...
//  dontTouch(dataRead)
    ...
    val dataHitWriteBus = Wire(CacheDataArrayWriteBus()).apply(
      data =
          Wire(new DataBundle).apply(MaskData(dataRead, req.wdata, wordMask)),
      valid = hitWrite,
      setIdx = Cat(addr.index, addr.wordIndex),
      waymask = waymask
    )
...
// class flushDCache(implicit val cacheConfig: SSDCacheConfig)
//     extends CacheModule {
...
//       data = Wire(new MetaBundle).apply(
